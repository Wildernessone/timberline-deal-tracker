...
export const BRAND_DOMAINS = {
  ...
  "Tethrd":"tethrd.com","Timber Ninja":"timberninjaoutdoors.com",
  "Hunter Safety System":"huntersafetysystem.com","Ozonics":"ozonicshunting.com",
  "Iron Will":"ironwilloutfitters.com","Slick Trick":"slicktrickbroadheads.com",
  "Day Six":"daysixgear.com","Gold Tip":"goldtip.com","Elite Archery":"elitearchery.com",
  "PSE Archery":"psearchery.com","Browning Trail Cameras":"browningtrailcameras.com",
  "Bushnell":"bushnell.com","GardePro":"gardepro.com","Insights Hunting":"insightshunting.com",
  "Antler King":"antlerking.com","Conquest Scents":"conquestscents.com",
  "Illusion Systems":"illusionsystems.com","Hunt Comfort":"huntcomfort.com",
  "Annihilator":"annihilatorbroadheads.com","Grim Reaper":"grimreaperbroadheads.com",
  "Magnus":"magnusbroadheads.com","Bee Stinger":"beestinger.com","UltraView":"ultraviewarchery.com",
  "Obsession Bows":"obsessionbows.com","Xpedition Archery":"xpeditionarchery.com",
  "Gamehide":"gamehide.com","Banks Outdoors":"banksoutdoors.com","Redneck Blinds":"redneckblinds.com",
  "Big Tine":"bigtine.com","Spartan Camera":"spartancamera.com","Dirt Nap Gear":"dirtnapgear.com",
  "Alaska Guide Creations":"alaskaguidecreations.com","Argali":"argalioutdoors.com",
  "Barronett Blinds":"barronettblinds.com","Domain Outdoor":"domainoutdoor.com",
  "DSG Outerwear":"dsgouterwear.com","Frigid Forage":"frigidforage.com",
  "Habit Outdoors":"habitoutdoors.com","Lathrop & Sons":"lathropandsons.com",
  "Lumenok":"lumenok.com","Morrell Targets":"morrelltargets.com","Muck Boot":"muckbootcompany.com",
  "North Mountain Gear":"northmountaingear.com","Prois":"proishunting.com","Rocky Boots":"rockyboots.com",
  "Scott Archery":"scottarchery.com","Work Sharp":"worksharptools.com","Grizzly Coolers":"grizzlycoolers.com",
};
...
// ── Durable category taxonomy (the "stays-green" link target) ──────────────────
// Articles/guides must NEVER deep-link a single /deal/<id> (it 404s the moment that
// sale ends). Instead they link to /category/<slug>, a listing that re-queries the
// LIVE active-deals set on every render — so "today's optics deals" stays accurate
// forever. Each group maps a stable, gear-concept slug to the raw `deals.cat` values
// the scraper actually writes (normalized lower+trim). Order = chip display order;
// `clothing` is the broad apparel umbrella and is matched LAST so a pants deal
// resolves to /category/pants, not /category/clothing.
export const CATEGORY_GROUPS = [
  {slug:"optics",       label:"Optics",            match:["optics"],                                      blurb:"binoculars, rangefinders, spotting scopes & riflescopes"},
  {slug:"jackets",      label:"Jackets & Outerwear", match:["insulation","wind layer"],                   blurb:"insulated, puffy, rain & wind-layer jackets"},
  {slug:"base-layers",  label:"Base Layers",       match:["base layer"],                                  blurb:"merino & synthetic base layers"},
  {slug:"pants",        label:"Pants & Bibs",      match:["pants"],                                       blurb:"hunting pants, bibs & waders"},
  {slug:"boots",        label:"Boots",             match:["boots"],                                       blurb:"hunting boots & footwear"},
  {slug:"packs",        label:"Packs",             match:["packs"],                                       blurb:"hunting packs, frames & bags"},
  {slug:"knives",       label:"Knives",            match:["knives"],                                      blurb:"fixed-blade & replaceable-blade knives"},
  {slug:"calls",        label:"Calls",             match:["calls"],                                       blurb:"game calls & vocalizations"},
  {slug:"blinds",       label:"Blinds & Shelter",  match:["shelter"],                                     blurb:"blinds, tents & shelter"},
  {slug:"accessories",  label:"Accessories",       match:["accessories"],                                 blurb:"hunting accessories & small gear"},
  {slug:"clothing",     label:"Clothing",          match:["clothing","insulation","base layer","pants","wind layer"], blurb:"hunting apparel & layering"},
];

const normalizeCat = (c) => String(c || "").toLowerCase().trim();

// Resolve a URL slug to its category group (or null for an unknown slug → 404).
export function categoryGroupBySlug(slug){
  return CATEGORY_GROUPS.find(g => g.slug === slug) || null;
}

// Does a deal's raw `cat` belong to this group?
export function dealInCategory(group, rawCat){
  if(!group) return false;
  return group.match.includes(normalizeCat(rawCat));
}

// Most-specific group for a raw `cat` value (umbrella `clothing` matched last).
// Used to soft-land an expired /deal/<id> on its live category page.
export function categoryGroupForCat(rawCat){
  const c = normalizeCat(rawCat);
  for(const g of CATEGORY_GROUPS){
    if(g.slug === "clothing") continue;
    if(g.match.includes(c)) return g;
  }
  return CATEGORY_GROUPS.find(g => g.slug === "clothing" && g.match.includes(c)) || null;
}
...
export const PORTALS = {
  ...
  whitetail: {
    id:"whitetail",name:"Treestand Saver",shortName:"Treestand Saver",domain:"treestandsaver.com",ogImage:"https://treestandsaver.com/og-treestand-saver.png",description:"Real saddle hunting & mobile-setup deals — saddles, platforms, climbing sticks, treestands & scent control from Tethrd, Trophyline, XOP and more. Updated every morning. No fake markdowns.",tagline:"Saddle · Mobile · Treestand",
    accent:"#7a4a2a",accentLight:"#f5ede4",accentBorder:"#d4b89a",panelAccent:"#c9a578",heroTitle:"Active Deals",heroBg:"#262420",heroTagline:"Real saddle-hunting and mobile-setup gear sales — tracked fresh every morning across saddles, platforms, climbing sticks, and the treestand and scent-control brands serious whitetail hunters run. No fake markdowns. No inflated MSRPs. Just the actual cheapest price online, right now.",favicon:`data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" fill="none"><rect width="64" height="64" rx="12" fill="%237a4a2a"/><g stroke="%23fffdf7" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" fill="none"><path d="M32 54 L32 34"/><path d="M32 34 L20 22 L18 12"/><path d="M32 34 L44 22 L46 12"/><path d="M22 24 L14 18"/><path d="M22 28 L13 28"/><path d="M26 20 L22 12"/><path d="M42 24 L50 18"/><path d="M42 28 L51 28"/><path d="M38 20 L42 12"/></g></svg>`,
    brands:["Sitka","First Lite","Keen","Katabatic Gear","Zpacks","Mtn Ops","Crispi","Yeti","Outdoor Research","Kings Camo","Marsupial Gear","Blue Coolers","Hoyt","Maven","Mathews","Forloh","Kryptek","Montana Knife Company","Kenetrek","Primos","Canvas Cutter","Outdoor Edge","Badlands","Wilderness Athlete","Pnuma Outdoors","Thermarest","Helinox","Duckworth","Chota Outdoor","Goat Knives","Darn Tough","FHF Gear","On Glass","GoHunt","Mystery Ranch","Vortex","Leupold","Swarovski","Garmin","onX","Sig Sauer","Schnees","SKRE Gear","TideWe","Mossy Oak","Bone Collector","Nomad Outdoor","XOP Outdoors","Novix Outdoors","Hunting Beast Gear","Lone Wolf Custom Gear","Trophyline","Tethrd","Timber Ninja","Eberlestock","Hunter Safety System","Ozonics","Iron Will","Slick Trick","Day Six","Gold Tip","Elite Archery","PSE Archery","Browning Trail Cameras","Bushnell","GardePro","Insights Hunting","Antler King","Conquest Scents","Illusion Systems","Hunt Comfort","Phelps Game Calls","Annihilator","Grim Reaper","Magnus","Bee Stinger","UltraView","Obsession Bows","Xpedition Archery","Gamehide","Banks Outdoors","Redneck Blinds","Big Tine","Spartan Camera","Dirt Nap Gear","Alaska Guide Creations","Argali","Barronett Blinds","Domain Outdoor","DSG Outerwear","Frigid Forage","Habit Outdoors","Lathrop & Sons","Lumenok","Morrell Targets","Muck Boot","North Mountain Gear","Prois","Rocky Boots","Scott Archery","Work Sharp","Grizzly Coolers","Filson","Drake Waterfowl","Buck Gardner","Latitude Outdoors","Smartwool","Leatherman","Spyderco"],
    searchHint:'Try "Tethrd Phantom" or "Iron Will broadhead"...',
    searchContext:"saddle hunting, tree saddle, mobile hunting, platform, climbing sticks, run and gun, whitetail deer, treestand, hang-on, climber, rut, eastern woods, midwest, scent control",
  },
  turkey: {
