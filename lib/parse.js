...
// ── Cross-product hero fallback (display-only) ──
// When a guide has no generated hero, reuse the best TOPIC-MATCHED hero from a
// pool of other published guides (elk→elk, whitetail→whitetail). Tries the OWN
// product's pool first, then the sibling product's (Timberline↔Timber&Tackle) to
// fill a gap — both directions. Stable per slug. NOT used for schema/sitemap, so
// we never claim a borrowed image as this article's own.
const HERO_STOP = new Set("the a an and or for to of in on with how best guide guides tips your you it is are vs review hunting hunt season setup buying gear guide".split(" "));
function topicTokens(a) {
  const out = new Set();
  const add = s => String(s || "").toLowerCase().split(/[^a-z0-9]+/).forEach(w => { if (w.length > 2 && !HERO_STOP.has(w)) out.add(w); });
  (Array.isArray(a.tags) ? a.tags : []).forEach(add);
  add(a.category); add(a.slug); add(a.title);
  return out;
}
function bestTopicMatch(article, pool, wanted) {
  let best = null, bestScore = 0;
  for (const c of pool || []) {
    if (!c || !c.hero_image || c.slug === article.slug) continue;
    const ct = topicTokens(c);
    let s = 0; for (const w of wanted) if (ct.has(w)) s++;
    if (s > bestScore) { bestScore = s; best = c; }
  }
  return bestScore > 0 ? best.hero_image : null;
}
export function pickMatchedHero(article, ownPool = [], siblingPool = []) {
  const wanted = topicTokens(article);
  // Topic match: own product first, then sibling (the cross-product gap-fill).
  const matched = bestTopicMatch(article, ownPool, wanted) || bestTopicMatch(article, siblingPool, wanted);
  if (matched) return matched;
  // No topical overlap anywhere → stable-hash any available hero (own, else sibling).
  const any = (ownPool.filter(c => c && c.hero_image && c.slug !== article.slug));
  const pool = any.length ? any : (siblingPool || []).filter(c => c && c.hero_image);
  if (!pool.length) return null;
  let h = 2166136261; const s = String(article.slug || "");
  for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); }
  return pool[(h >>> 0) % pool.length].hero_image;
}
