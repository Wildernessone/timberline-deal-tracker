...
export async function getArticlesForSitemap(portalId = PORTAL.id) {
  const rows = await sbFetch(
    "articles",
    { select: "slug,updated_at,published_at,hero_image", portal: "eq." + portalId, status: "eq.published", order: "published_at.desc", limit: "1000" },
    { tags: ["articles"] }
  );
  return rows || [];
}

// Recent published guides WITH full body — for the editorial RSS feed (MSN
// Partner Hub / Microsoft Start ingestion wants full content + an image).
export async function getArticlesForFeed(portalId = PORTAL.id, { limit = 30 } = {}) {
  const rows = await sbFetch(
    "articles",
    { select: "slug,title,dek,body_md,hero_image,author,tags,published_at,updated_at", portal: "eq." + portalId, status: "eq.published", order: "published_at.desc", limit: String(limit) },
    { tags: ["articles", "articles:" + portalId] }
  );
  return rows || [];
}

// Hero fallback pools (display-only; see pickMatchedHero in lib/parse.js).
// Own pool = every published HUB guide that has a hero, across all portals (so an
// elk guide can borrow an elk hero from any Timberline portal).
export async function getHubHeroPool({ limit = 100 } = {}) {
  const rows = await sbFetch(
    "articles",
    { select: "slug,title,tags,hero_image,portal", status: "eq.published", hero_image: "not.is.null", order: "published_at.desc", limit: String(limit) },
    { tags: ["articles"] }
  );
  return rows || [];
}

// Sibling pool = Timber & Tackle's published heroes (public anon read; both are
// public keys). Best-effort — a brand-new portal with no own heroes borrows here.
const TT_HERO_URL = "https://pkhidglneuuodzujqgip.supabase.co";
const TT_HERO_ANON = "sb_publishable_cEx3Oz8FLZ2zInkEicSUWA_wOoIKwp0";
export async function getTTHeroPool({ limit = 100 } = {}) {
  try {
    const r = await fetch(
      `${TT_HERO_URL}/rest/v1/articles?status=eq.published&hero_image=not.is.null&select=slug,title,category,hero_image&order=published_at.desc&limit=${limit}`,
      { headers: { apikey: TT_HERO_ANON, Authorization: "Bearer " + TT_HERO_ANON }, next: { revalidate: 3600, tags: ["articles"] } }
    );
    if (!r.ok) return [];
    const rows = await r.json();
